...
const SongMapClient = ({ allPlaylists, allSongs }: SongMapClientProps) => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest('[data-song-node]')) {
      return;
    }
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    if (isPanning && !e.target.closest('[data-song-node]')) {
        ...
        <svg width={MAP_SIZE} height={MAP_SIZE} className="absolute top-0 left-0 pointer-events-none">
          <g>
            {connections.map(conn => {
              const isVisible = !selectionDetails || (conn.songIds.includes(selectionDetails.selectedSong.id) && conn.songIds.some(id => selectionDetails.connectedSongIds.has(id)));
