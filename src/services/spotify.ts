...
async function getAccessToken(): Promise<string> {
  ...
  throw new Error('No valid Spotify access token found. Please sign in with Spotify.');
}
...
async function fetchSpotifyWithToken(token: string, endpoint: string, options: RequestInit = {}) {
  const response = await fetch(`${SPOTIFY_API_BASE}${endpoint}`, {
    ...options,
    headers: {
      ...options.headers,
...
async function fetchAllPlaylistTracksWithToken(token: string, playlistId: string): Promise<any[]> {
    let allItems: any[] = [];
    let nextUrl: string | null = `/playlists/${playlistId}/tracks?fields=items(added_at,track(id,name,artists,album(images),popularity)),next&limit=100`;

    while (nextUrl) {
        const data = await fetchSpotifyWithToken(token, nextUrl);
        if (data && Array.isArray(data.items)) {
            allItems = allItems.concat(data.items);
        }
        nextUrl = data?.next ? data.next.replace(SPOTIFY_API_BASE, '') : null;
...
export async function getPlaylistsWithTracksUsingToken(token: string): Promise<Playlist[]> {
  const userProfile = await fetchSpotifyWithToken(token, '/me');
  const userId = userProfile.id;

  const playlistData = await fetchSpotifyWithToken(token, '/me/playlists?limit=50');
  ...
  for (const [index, p] of userOwnedOrCollaborativePlaylists.entries()) {
      ...
      const allTrackItems = await fetchAllPlaylistTracksWithToken(token, p.id);
